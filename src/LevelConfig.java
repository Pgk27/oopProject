...
public class LevelConfig {
    ...
    public static LevelConfig getLevelConfig(int level) {
        ...
        if (level == 1) {
            ...
            w1.addSpawn(new MobSpawnInfo(Value.mobMonster2, 12, 2000, 1.0, 1.0, 1.0));
            w1.addSpawn(new MobSpawnInfo(Value.mobMonster1, 12, 2000, 1.0, 1.0, 1.0)); // Slime
            config.waves.add(w1);

            // Wave 2 (0:30 - 0:55): 8x Slime + 3x Orc xuất hiện xen kẽ (Spawn mỗi 1.5s).
            WaveData w2 = new WaveData(30000); // 30s
            w2.addSpawn(new MobSpawnInfo(Value.mobMonster2, 14, 2000, 1.0, 1.0, 1.0)); // Slime
            w2.addSpawn(new MobSpawnInfo(Value.mobMonster1, 14, 2000, 1.0, 1.0, 1.0)); // Orc
            config.waves.add(w2);
